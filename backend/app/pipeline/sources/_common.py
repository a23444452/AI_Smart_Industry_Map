...
def get_json_dict(url: str, *, source: str) -> dict:
    ...
    The TWSE ``rwd`` (T86, BFI82U, MI_MARGN) and TPEx ``dailyTrade`` endpoints
    wrap their rows in a ``{stat, fields, data}`` / ``{stat, tables:[...]}``
    object rather than the bare array that OpenAPI serves, so ``get_json``
    (list-only) cannot be used. Same friendly-error contract: raises
    SourceFetchError on any non-200, connection/timeout failure, unparseable
    body, or a 200 that is not a dict.
    """
    try:
        with httpx.Client(
            timeout=TIMEOUT_SECONDS, follow_redirects=True, verify=_ssl_context()
