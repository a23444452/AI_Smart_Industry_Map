...
def _seed_announcements(client) -> None:
    """台北 2026-07-12 當日三筆（含一筆澄清回應）＋前一日、隔日各一筆。"""
    _add(
        client,
        [
            # 台北 2026-07-12 當日（UTC 7/11 16:30、7/12 03:00、7/12 05:00），降冪
...
def test_announcements_by_taipei_date_all_categories_and_orders(client):
    _seed_announcements(client)
    # 未帶 category → 回該日全部分類（含澄清回應），published_at 降冪
    r = client.get("/api/daily/announcements?date=2026-07-12")
    assert r.status_code == 200
    items = r.json()
    assert [i["title"] for i in items] == ["澄清", "晚", "早"]
    first = items[0]
    ...
    assert first["category"] == "澄清回應"
    assert first["published_at"] == "2026-07-12T05:00:00Z"  # 帶 Z
    assert items[1]["ticker"] == "3443"
    assert items[1]["name"] == "創意"


def test_announcements_category_param_filters(client):
    _seed_announcements(client)
    # 帶 category → 只回該分類（仍限當日範圍）
    r = client.get("/api/daily/announcements?date=2026-07-12&category=澄清回應")
    assert r.status_code == 200
    assert [i["title"] for i in r.json()] == ["澄清"]

    r = client.get("/api/daily/announcements?date=2026-07-12&category=重大事件")
    assert [i["title"] for i in r.json()] == ["晚", "早"]


def test_announcements_unknown_category_returns_empty(client):
    _seed_announcements(client)
    r = client.get("/api/daily/announcements?date=2026-07-12&category=不存在分類")
    assert r.status_code == 200
    assert r.json() == []
